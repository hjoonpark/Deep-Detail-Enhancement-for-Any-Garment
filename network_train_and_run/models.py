...
class Generator_CNNCIN_nvidia(nn.Module):
    def __init__(self, inDim, outDim, styleNum=0, ifADD=True):
        super(Generator_CNNCIN_nvidia, self).__init__()
        self.ifADD = ifADD
        self.E1_Conv2d = nn.Conv2d(32, 64, kernel_size=3, stride=1, padding=1)
        ...
        self.out_Conv2d = nn.Conv2d(32, outDim, kernel_size=3, stride=1, padding=1) if ifADD is True else \
            nn.ConvTranspose2d(32+32, outDim, kernel_size=3, stride=1, padding=1)

        self.E0_Conv2d = nn.Conv2d(inDim, 32, kernel_size=3, stride=1, padding=1)
        self.E0_ReLu = nn.ReLU(True)
        self.E0_Pool = nn.MaxPool2d(kernel_size=2, stride=2, padding=0, dilation=1,
                                    ceil_mode=False)  # [B, 64, H//2, W//2], Default H = 512, W = 512

        self.D5_Deconv2d = nn.ConvTranspose2d(64, 32, kernel_size=4, stride=2, padding=1) if ifADD is True else \
            nn.ConvTranspose2d(64+64, 32, kernel_size=4, stride=2, padding=1)  # [B, 64, H, W]
        self.D5_CIN = batch_InstanceNorm2d(styleNum, 32)
        self.D5_ReLu = nn.ReLU(True)

    def forward(self, X, styleId=torch.IntTensor([0])):
        e0 = self.E0_Conv2d(X)
        e0 = self.E0_ReLu(e0)  # [B, 32, H, W]
        e1a = self.E0_Pool(e0)  # [B, 32, H//2, W//2]

        e1 = self.E1_Conv2d(e1a)
        ...
        d5 = self.D5_Deconv2d(d4)  # [B, 64, H, W]
        if self.ifADD:
            d5 = self.D5_CIN(d5 + e0, styleId)
            d5 = self.D5_ReLu(d5)
        else:
            d5 = self.D5_ReLu(self.D5_CIN(d5, styleId))
            d5 = torch.cat([d5, e0], dim=1)

        y = self.out_Conv2d(d5)
        return y
